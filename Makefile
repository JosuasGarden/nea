...
run-backend-docker:
	CGO_ENABLED=0 nodemon --config dev/nodemon.json --signal SIGTERM --exec "go run" -ldflags="-s -w -X 'main.buildString=${BUILDSTR}' -X 'main.versionString=${VERSION}' -X 'main.frontendDir=frontend/dist'" cmd/*.go --config=dev/config.toml
